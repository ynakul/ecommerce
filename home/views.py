...
def usedproductView(request, myid):
    usedproduct=UsedProduct.objects.filter(id=myid)
    print(usedproduct)
    return render(request, "usedproductpage.html", {'usedproduct':usedproduct[0]})
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated and Customer.objects.filter(user=request.user).exists():
            ...
            if request.user.customer != order.cart.customer:
                return redirect("books:customerprofile")
        else:
            return redirect("/login/?next=/profile/")
        return super().dispatch(request, *args, **kwargs)
...
class AdminLoginView(FormView):
    ...
    form_class = CustomerLoginForm
    success_url = reverse_lazy("books:adminhome")
...
class AdminOrderStatuChangeView(AdminRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        order_obj.save()
        return redirect(reverse_lazy("books:adminorderdetail", kwargs={"pk": order_id}))
...
class AdminProductCreateView(AdminRequiredMixin, CreateView):
    ...
    form_class = ProductForm
    success_url = reverse_lazy("books:adminproductlist")
